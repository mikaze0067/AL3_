...
void Player::Update() {
	...
	ChecMapCollisionDown(collisionMapInfo);
...
void Player::CheckMapLanding(const CollisionMapInfo& info) {
	...
	if (onGround_) {
		...
		if (velocity_.y > 0.0f) {
			...
			//
			float magicNumber = 1.001f;

			// 左下点の判定
			MapChipField::IndexSet indexSet;
			indexSet = mapChipField_->GetMapChipIndexSetByPosition(CornerPosition(worldTransform_.translation_ + info.move, kLeftBottom) + Vector3(0, -magicNumber, 0));
			...
			// 右下点の判定
			indexSet = mapChipField_->GetMapChipIndexSetByPosition(CornerPosition(worldTransform_.translation_ + info.move, kRightBottom) + Vector3(0, -magicNumber, 0));
			mapChipType = mapChipField_->GetMapChipTypeByIndex(indexSet.xIndex, indexSet.yIndex);
			if (mapChipType == MapChipType::kBlock) {
				hit = true;
			}
